...
const Favorite = () => {
  ...
  const handleDeleteFavorite = (favoriteId) => {
    dispatch(deleteFavorite(favoriteId)).then((result) => {
      if (deleteFavorite.fulfilled.match(result)) {
        ...
        toast.error("Không thể xóa mục yêu thích");
      }
    });
  };
  ...
  const handleToggleObserved = (favoriteId, currentObserved) => {
    dispatch(toggleObserved({ favoriteId, currentObserved })).then((result) => {
      if (toggleObserved.fulfilled.match(result)) {
        const newObserved = result.payload.newObserved;
        toast.success(`Thông báo đã được ${newObserved ? "bật" : "tắt"}`);
      } else {
      ...
      >
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            aria-label={
              fav.isObserved
                ? "Tắt thông báo cho mục yêu thích này"
                : "Bật thông báo cho mục yêu thích này"
            }
          >
            {fav.isObserved ? "Tắt" : "Bật"}
          </button>
          <button
            onClick={() => handleDeleteFavorite(fav.id)}
            className="px-3 py-1 text-sm text-white bg-red-500 rounded hover:bg-red-600 transition-colors"
            aria-label="Xóa mục yêu thích này"
          >
            Xóa
          </button>
        </div>
  ...
  const renderScheduleItem = (fav) => {
    const { routeVariant, startDate } = fav.targetData || {};
    if (!routeVariant?.name || !routeVariant?.startStop || !routeVariant?.endStop) return null;

    // Format startDate nếu có
    const formattedDate = startDate
      ? new Date(startDate[0], startDate[1] - 1, startDate[2]).toLocaleDateString("vi-VN")
      : "Không xác định";
    ...
    return (
      ...
      >
        <div className="flex-1">
          <span className="text-sm font-semibold text-blue-600">LỊCH TRÌNH</span>
          <div className="mt-1">
            <span className="font-bold text-gray-800">{routeVariant.name}</span>
            <span className="text-sm text-gray-500"> (Bắt đầu: {formattedDate})</span>
          </div>
          <div className="mt-1 text-sm text-gray-600">
            {routeVariant.startStop} → {routeVariant.endStop}
          </div>
  ...
  const renderFavorites = () => {
    ...
    if (favorites.length === 0) {
      return (
        <p className="text-center text-gray-500 py-8">
          Không tìm thấy mục yêu thích cho {activeTab === "ROUTE" ? "tuyến đường" : "lịch trình"}.
        </p>
      );
    }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-gray-50 min-h-screen">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">Danh sách yêu thích</h2>
      <div className="flex mb-6 border-b border-gray-200">
        <button
          onClick={() => setActiveTab("ROUTE")}
          className={`px-4 py-2 text-sm font-medium transition-colors ${
